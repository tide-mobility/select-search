...
export default function SearchPage() {
  ...
  const [results, setResults] = useState<SearchResult[]>([]); // SearchResult 타입 배열로 명시
  ...
  const handleSearch = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select("*")
        .or(
          `property_name_kor.ilike.%${query}%,property_name_eng.ilike.%${query}%,property_details.ilike.%${query}%,blog_title.ilike.%${query}%,blog_sub_title.ilike.%${query}%,blog_contents.ilike.%${query}%`
        );
      if (error) throw error;
      setResults(data || []);
    } catch (err: Error | PostgrestError) {
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div className="w-full max-w-xl bg-white rounded-lg shadow-md p-6">
        ...
        <div className="mt-4">
          {/* Property Results */}
          {results.filter(
            (result) => result.property_name_kor || result.property_name_eng
          ).length > 0 ? (
            <div>
          ...
          {/* Blog Results */}
          {results.filter(
            (result) => result.blog_title || result.blog_sub_title
          ).length > 0 ? (
            <div className="mt-4">
