  ...
  const handleSearch = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('*')
        .or(`sabre_id.ilike.%${query}%,property_name_eng.ilike.%${query}%,paragon_id.ilike.%${query}%`);
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div className="w-full max-w-xl bg-white rounded-lg shadow-md p-6">
        <h1 className="text-2xl font-bold mb-4">Sabre 숙소 시설, ID 검색 ver 0.001</h1>
...
<div className="mt-4">
        {/* Property Results */}  
        {results.filter(result => result.sabre_id || result.property_name_eng).length > 0 ? (
          <div>
            ...
            <ul className="divide-y divide-gray-200">
              {results.map((result) => (
                result.sabre_id || result.property_name_eng ? (
                  <li key={result.id} className="py-4">
                    <h2 className="text-lg font-semibold">{result.property_name_eng}</h2>
                    <p className="text-gray-600">Sabre ID: {result.sabre_id}</p>
                    <p className="text-gray-600">Paragon ID: {result.paragon_id}</p>
                  </li>
                ) : null
              ))}
            </ul>
